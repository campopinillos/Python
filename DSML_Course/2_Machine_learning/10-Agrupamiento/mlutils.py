...
def plot_2D_boundary(predict, mins, maxs, line_width=3, line_color="black", line_alpha=1, label=None):
    ...
    levels = [np.min(levels)-1] + [np.mean(levels[i:i+2])
                                   for i in range(len(levels)-1)] + [np.max(levels)+1]
    p = (preds*1.).reshape((n, n))
    plt.contour(gd0, gd1, p, levels=levels, alpha=line_alpha,
...
def plot_2Ddata_with_boundary(predict, X, y, line_width=3, line_alpha=1, line_color="black", dots_alpha=.5, label=None, noticks=False):
    ...
    p0, p1 = plot_2D_boundary(
        predict, mins, maxs, line_width, line_color, line_alpha, label)
    if noticks:
        plt.xticks([])
        plt.yticks([])
...
def plot_2Ddata(X, y, dots_alpha=.5, noticks=False):
    ...
    for i, label in enumerate(np.unique(y)):
        plt.scatter(X[y == label][:, 0], X[y == label][:, 1],
                    color=colors[i], alpha=dots_alpha)
    if noticks:
        plt.xticks([])
        plt.yticks([])
...
class Example_Bayes2DClassifier():
    ...
    def analytic_score(self):
        ...
        if rng == rngs[-1]:
            print("warning: bounding box prob size",
                  rng, "has prob", np.max([bp0, bp1]))
...
def plot_cluster_predictions(clustering, X, n_clusters=None, cmap=plt.cm.plasma,
    ...
    assert not hasattr(clustering, "n_clusters") or \
        (hasattr(clustering, "n_clusters")
         and n_clusters is not None), "must specify `n_clusters` for "+str(clustering)
    ...
    if plot_data:
        plt.scatter(X[:, 0], X[:, 1], color=cmap(
            (y*255./(n_clusters-1)).astype(int)), alpha=.5)
    if plot_centers and hasattr(clustering, "cluster_centers_"):
        plt.scatter(clustering.cluster_centers_[:, 0], clustering.cluster_centers_[:, 1], s=150,  lw=3,
                    facecolor=cmap(
                        (np.arange(n_clusters)*255./(n_clusters-1)).astype(int)),
                    edgecolor="black")
...
def experiment_KMeans_number_of_iterations(X, n_clusters=3,
    ...
    for i in range(10):
        ...
        if i == 0:
            ...
            y = np.argmin(np.vstack(
                [np.sqrt(np.sum((X-i)**2, axis=1)) for i in init_centroids]).T, axis=1)

            plt.scatter(X[:, 0], X[:, 1], color=cm(
                (y*255./(n_clusters-1)).astype(int)), alpha=.5)
            plt.scatter(init_centroids[:, 0], init_centroids[:, 1], s=150,  lw=3,
                        facecolor=cm((np.arange(n_clusters)*255. /
                                      (n_clusters-1)).astype(int)),
                        edgecolor="black")
            plt.axis("off")
            ...
            km = KMeans(n_clusters=n_clusters, init=init_centroids,
                        n_init=1, max_iter=2*n_iterations)
            km.fit(X)

            plot_cluster_predictions(
                km, X, n_clusters, cm, plot_data, plot_centers, plot_boundaries)
